---
layout:     post
title:      二进制入门--动态跟踪源代码与反汇编代码
date:       2016-06-04
summary:    栈基础知识与反汇编代码理解
categories: 二进制
tags:
 - stack
 - linux
...
#### 1>https://blog.chinaunix.net/uid-1848701-id-2825699.html
...
图frame
<img src="https://raw.githubusercontent.com/3xp10it/pic/master/eip.png">

#### 5>leave===>mov esp,ebp + pop ebp

#### 6>retn===>pop eip + jmp eip    (eip=[esp],esp=esp+4)

```
分析ret汇编语句时,最好不要把ret分开当作两句汇编指令,这样容易因为人脑习惯逐句解析两句,会和计算机的解析一句汇编指令错乱
特别是影响esp,eip的寄存器的分析时,要按照计算机的解析模式-->ret只是一句指令,执行完ret指令后,eip=[esp],esp=esp+4
分析ret指令时,先分析eip,再分析esp
```

#### 7>retn 0xn===>retn && esp=esp+0xn===>eip=[esp] && esp=esp+4+0xn
...
```
由于call addr等同于push eip+jmp addr,执行call esp时,esp是个地址,系统将这里的esp翻译成一个普通地址,假设值为addr,call esp即相当于上面的call addr=push eip+jmp addr
      代码段:                                    栈段:          
                      ..                                                
                      ..                                  | addr1 |
...
#### 9>pop pop ret=jmp [esp+16](条件:pop pop ret的地址在栈中且马上要执行)
...
#### 1>gdb常用调试命令
    ...
    https://wenku.baidu.com/link?url=W1u-DTrFwMreJsYavwKcI_yDzJsJglUJ4yfcXXXxWOkGTlJSSqXm-4zohcUYxGciOIeFRcSY0gr2qn-K5pnqGRU6nGbgQvW6XVfaRMCdmKq
...
图ascii
<img src="https://raw.githubusercontent.com/3xp10it/pic/master/ascii.png">
...
图stack0
<img src="https://raw.githubusercontent.com/3xp10it/pic/master/stack0.png">
...
解决方法:(https://blog.csdn.net/nuoline/article/details/8610694)
ulimit -c
---------output:----------
0
------------end-----------
...
换工具,rop-tool
https://www.freebuf.com/sectool/105715.html
先安装上面链接中说的capstone
https://www.capstone-engine.org/download/3.0.4/ubuntu-14.04/python-capstone_3.0.4-0.1ubuntu1_amd64.deb
然后下载rop-tool
...
<img src="https://raw.githubusercontent.com/3xp10it/pic/master/opcodesearcher.png">
...
下载checksec.sh检查一下:https://www.trapkit.de/tools/checksec.sh(或者peda工具,https://github.com/3xp10it/peda)
chmod +x ./checksec.sh
